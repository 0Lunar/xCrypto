...
static void _sha1_digest(struct _xcrypto_sha1_ctx *ctx) {
    uint32_t A;
    uint32_t B;
    uint32_t C;
    uint32_t D;
...
struct _xcrypto_sha1_ctx *XSHA1_Init() {
    struct _xcrypto_sha1_ctx *ctx;

    if ((ctx = malloc(sizeof(struct _xcrypto_sha1_ctx))) == NULL)
        return NULL;
    ...
    ctx->buf_len = 0;
    ctx->bits = 0;
    ctx->error = SHA1_SUCCESS;

    return ctx;
}


enum _xcrypto_sha1_errors XSHA1_Update(struct _xcrypto_sha1_ctx *ctx, const uint8_t *buf, size_t bufSize) {
    if (!ctx)
        return SHA1_ERR_NULL_PTR;
    
    if (!buf) {
        ctx->error = SHA1_ERR_NULL_PTR;
        return SHA1_ERR_NULL_PTR;
    }
...
enum _xcrypto_sha1_errors XSHA1_Finalize(struct _xcrypto_sha1_ctx *ctx, uint8_t *buf) {
    if (!ctx)
        return SHA1_ERR_NULL_PTR;
...
enum _xcrypto_sha1_errors XSHA1_Reset(struct _xcrypto_sha1_ctx *ctx) {
    ctx->H0 = 0x67452301;
    ctx->H1 = 0xEFCDAB89;
    ctx->H2 = 0x98BADCFE;
    ctx->H3 = 0x10325476;
