...
static void _sha224_digest(struct _xcrypto_sha224_ctx *ctx) {
    uint32_t A;
    uint32_t B;
    uint32_t C;
    uint32_t D;
...
struct _xcrypto_sha224_ctx *XSHA224_Init() {
    struct _xcrypto_sha224_ctx *ctx;

    if ((ctx = malloc(sizeof(struct _xcrypto_sha224_ctx))) == NULL)
        return NULL;
...
enum _xcrypto_sha224_errors XSHA224_Update(struct _xcrypto_sha224_ctx *ctx, const uint8_t *buf, size_t bufSize) {
    if (!ctx)
        return SHA224_ERR_NULL_PTR;
    
    if (!buf) {
        ctx->error = SHA224_ERR_NULL_PTR;
        return SHA224_ERR_NULL_PTR;
    }
...
enum _xcrypto_sha224_errors XSHA224_Finalize(struct _xcrypto_sha224_ctx *ctx, uint8_t *buf) {
    if (!ctx)
        return SHA224_ERR_NULL_PTR;
