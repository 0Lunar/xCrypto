...
FORCE_INLINE void _add_uint128(struct _xcrypto_sha512_ctx *ctx, uint64_t num) {
    uint64_t tmp;
...
static void _sha512_digest(struct _xcrypto_sha512_ctx *ctx) {
    uint64_t A;
    uint64_t B;
    uint64_t C;
    uint64_t D;
...
struct _xcrypto_sha512_ctx *XSHA512_Init() {
    struct _xcrypto_sha512_ctx *ctx;

    if ((ctx = malloc(sizeof(struct _xcrypto_sha512_ctx))) == NULL)
        return NULL;
...
enum _xcrypto_sha512_errors XSHA512_Update(struct _xcrypto_sha512_ctx *ctx, const uint8_t *buf, size_t bufSize) {
    if (!ctx)
        return SHA512_ERR_NULL_PTR;
    
    if (!buf) {
        ctx->error = SHA512_ERR_NULL_PTR;
        return SHA512_ERR_NULL_PTR;
    }
...
enum _xcrypto_sha512_errors XSHA512_Finalize(struct _xcrypto_sha512_ctx *ctx, uint8_t *buf) {
    if (!ctx)
        return SHA512_ERR_NULL_PTR;
